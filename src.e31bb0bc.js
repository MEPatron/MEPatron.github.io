...
function appendGraphLabels(g) {
  ...
  g.append('text').text('Prix (CAD) 💡').attr('class', 'x axis-text').attr('font-size', 15).style("text-shadow", "none").style("cursor", "pointer").on('click', function () {
    window.open('https://www.saq.com/fr/prix-des-produits', '_blank' // in a new tab
    );
  });
}
/**
...
var MIN_UNIT_TO_KEEP = 1; // 0 bottles == out of stock in Quebec, so ignore
...
function convertPercentileRange(percentileRange) {
  ...
  var range = [Math.round(N - percentileRange[1] / 100.0 * N), Math.round(N - (percentileRange[0] - 1) / 100.0 * N)];
  return range;
}
...
function getFilteredProducts(regionName, succID, filter, lowerRange, upperRange) {
  ...
  return getTopProduits([lowerRange, upperRange]).filter(function (prod) {
    return !removedCategories.has(prod.Cat1);
  });
}
/*
...
function init() {
  ...
  d3.select('#modal-bg').on('click', closeFct);
}
/**
 * @param id
 */
...
},{}],"scripts/map/helper.js":[function(require,module,exports) {
"use strict";
...
exports.initMapNav = initMapNav;
exports.initSuccTypeObject = initSuccTypeObject;
...
function initPanelDiv() {
  d3.select('#map-panel').style('width', '215px').style('border', '1px solid black').style('padding', '10px');
}
/**
...
function initSwitch(viz, legend, colorScale, graphSize, margin, succTypes) {
  d3.select('#switch-map').on('click', function (d) {
    ...
    if (d3.select(this).html() === '<i class="fas fa-sync-alt"></i>&nbsp;...par 100 000 habitants?') {
      ...
      d3.select('.legend-title-bg').attr('width', 215); // change map regions fill
      ...
      d3.select('.legend-title-bg').attr('width', 255); // change map regions fill
...
function getProjection() {
  return d3.geoMercator().center([-72.304095, 52.277163]).scale(2600);
}
/**
...
function initMapNav(svg, zoom) {
  d3.select('#map-reset-bg').remove();
  d3.select('#map-reset-icon').remove();
  d3.select('#map-reset-btn').remove();
  d3.select('#map-viz svg').append('rect').attr('x', document.getElementById('svg-map').clientWidth - 50).attr('y', 50).attr('rx', 3).attr('ry', 3).attr('width', 30).attr('height', 30).attr('fill', 'rgb(255,255,255)').attr('filter', 'url(#shadow-2)').attr('id', 'map-reset-bg');
  d3.select('#map-viz svg').append('svg').attr('x', document.getElementById('svg-map').clientWidth - 47).attr('y', 53).attr('width', 30).attr('height', 30).attr('id', 'map-reset-path').append('path').attr('id', 'map-reset-icon').attr('d', 'M14 12c0-1.1-.9-2-2-2s-2 .9-2 2 .9 2 2 2 2-.9 2-2zm-2-9c-4.97 0-9 4.03-9 9H0l4 4 4-4H5c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.51 0-2.91-.49-4.06-1.3l-1.42 1.44C8.04 20.3 9.94 21 12 21c4.97 0 9-4.03 9-9s-4.03-9-9-9z');
  d3.select('#map-viz svg').append('rect').attr('x', document.getElementById('svg-map').clientWidth - 50).attr('y', 50).attr('rx', 3).attr('ry', 3).attr('width', 30).attr('height', 30).attr('fill', 'transparent').attr('id', 'map-reset-btn').on('mouseover', function () {
    d3.select(this).style('cursor', 'pointer');
    d3.select('#map-reset-icon').attr('fill', 'grey');
  }).on('mouseout', function () {
    d3.select('#map-reset-icon').attr('fill', 'black');
  }).on('click', function () {
    svg.transition().duration(500).call(zoom.transform, d3.zoomIdentity.scale(1));
  }).append('title').text('Recentrer la carte');
}
/**
...
function initSuccTypeObject(data, succTypes) {
...
function getRegionDesc(d) {
  ...
  if (!(succID != "all" && regionName == "all") && d.SuccDispoCount != 0) {
...
function drawLegend(colorScale, sizeScale, updateFunc, g, dims) {
  // color legend
  var legend = g.append("g").attr("class", "legendOrdinal");
  ...
  }).style("cursor", "pointer"); //legend background (to avoid reading interference from the bubbles)
  ...
  g.select("#legend-background").attr("width", bbox.width + bboxMargin + 10).attr("height", bbox.height + bboxMargin + 50).attr("x", bbox.x - bboxMargin / 3).attr("y", bbox.y - bboxMargin / 2); // add size scale to legend

  var sizeData = [1, 10, 100, 1000, 10000, 100000];
  g.selectAll(".scaleCircle").data(sizeData).enter().append("circle").attr("cx", function (d, i) {
    return dims.full_width - bbox.width - 47 + 33 * i;
  }).attr("cy", dims.top + bbox.y + bbox.height + 35).attr("r", function (d) {
    return sizeScale(d);
  }) // .attr("fill", "black")
  .attr("class", "scaleCircle");
  g.selectAll(".scaleText").data(sizeData).enter().append("text").attr("class", "scaleText").attr("x", function (d, i) {
    return dims.full_width - bbox.width - 50 + 30 * i;
  }).attr("y", dims.top + bbox.y + bbox.height + 55).text(function (d) {
    return d;
  }).style("font-size", 10);
  g.append("text").attr("class", "scaleDesc").attr("x", dims.full_width - bbox.width - 44).attr("y", dims.top + bbox.y + bbox.height + 20).text("Inventaire au Québec (unités)").style("font-size", 12).style("font-weight", "bold");
}
},{"d3-svg-legend":"../node_modules/d3-svg-legend/indexRollupNext.js","./preprocess.js":"scripts/bubblechart/preprocess.js"}],"scripts/bubblechart/range-select.js":[function(require,module,exports) {
"use strict";
...
var selectedRange = [50, 100];

function add(id, updateFct, dims) {
  ...
  var range = [min, max];
  var ratioPercentile = 100 / max;
  var beginRange = [max / 2, max]; // set width and height of svg
  ...
  var height = h - margin.top - margin.bottom; // create x scale

  var x = d3.scaleLinear().domain(range) // data space
  .range([0, width]); // display space
  // create svg and translated g

  var svg = d3.select("#" + id).append("svg").attr("width", w).attr("height", h).attr("id", "bubbleRangeSelect");
  var g = svg.append('g').attr('transform', "translate(".concat(margin.left, ", ").concat(margin.top, ")")); // draw background lines

  g.append('g').selectAll('line').data(d3.range(range[0], range[1] + 1)).enter().append('line').attr('x1', function (d) {
    return x(d);
  }).attr('x2', function (d) {
    return x(d);
  }).attr('y1', 0).attr('y2', height).style('stroke', '#aaa'); // labels

  var labelL = g.append('text').attr('id', 'labelleft').attr('x', 0).attr('y', height + 5).text(range[0]);
  var labelR = g.append('text').attr('id', 'labelright').attr('x', 0).attr('y', height + 5).text(range[1]);

  var getRange = function getRange() {
    return d3.brushSelection(gBrush.node()).map(function (d) {
      return Math.round(x.invert(d));
    });
  }; // define brush


  var brush = d3.brushX().extent([[0, 0], [width, height]]).on('brush', function () {
    var s = d3.event.selection; // update and move labels

    labelL.attr('x', s[0]).text(ratioPercentile * Math.round(x.invert(s[0])));
    labelR.attr('x', s[1]).text(ratioPercentile * Math.round(x.invert(s[1]))); // move brush handles      
    ...
    selectedRange = getRange().map(function (x) {
      return x * ratioPercentile;
    });
    updateFct();
  }); // append brush to g

  var gBrush = g.append("g").attr("class", "brush").call(brush); // add brush handles (from https://bl.ocks.org/Fil/2d43867ba1f36a05459c7113c7f6f98a)
  ...
  var handle = gBrush.selectAll(".handle--custom").data([{
    type: "w"
  }, {
    type: "e"
  }]).enter().append("path").attr("class", "handle--custom").attr("stroke", "#000").attr("fill", '#eee').attr("cursor", "ew-resize").attr("d", brushResizePath); // override default behaviour - clicking outside of the selected area 
  // will select a small piece there rather than deselecting everything
  // https://bl.ocks.org/mbostock/6498000

  gBrush.selectAll(".overlay").each(function (d) {
    d.type = "selection";
...
},{}],"scripts/bubblechart/viz.js":[function(require,module,exports) {
"use strict";
...
var rangeSelect = _interopRequireWildcard(require("./range-select.js"));
...
function init() {
  dims = {
    ...
    width: 0,
    height: 0,
    full_width: window.innerWidth - 35,
    //35 == scroll bar width
  ...
  var svg = d3.select("#BubbleChartSection").append("svg").attr("id", "svgBubble").attr("width", dims.width + dims.left + dims.right).attr("height", dims.height + dims.top + dims.bottom);
  svg.append("g").attr("transform", "translate(" + dims.left + "," + 0 + ")").attr("width", dims.width).attr("height", dims.height + dims.bottom * 2).attr("id", "bubbleArea"); // clip path to restrict points to the chart's rect (between the axis)

  d3.select("#svgBubble").append("clipPath").attr("id", "products-clip").append("rect").attr("width", dims.width).attr("height", dims.height);
  d3.select("#bubbleArea").attr("clip-path", "url(#products-clip)"); // tooltip

  var productTip = (0, _d3Tip.default)().attr('class', 'd3-tip').html(function (d) {
  ...
  svg.call(productTip);
  var data = Object.values(preprocess.getProduits()); //  scales x,y & bubble size

  ORIGINAL_YSCALE = d3.scaleLinear().domain([0, 1]).range([dims.height, 0]).nice();
  ORIGINAL_XSCALE = d3.scaleLog().domain([1.90, d3.max(data, function (d) {
    return d.prix;
  }) + 100]).range([0, dims.width]);
  var maxRange = dims.width * dims.height / 100000; // dynamically determine proper bubble size 

  var sizeScale = d3.scaleLog().domain([1, d3.max(data.map(function (d) {
    return d.total;
  }))]).range([2, maxRange]);
  var colorScale = preprocess.getColorScale1();
  addZoom(dims, dims.height); // labels
...
function getBubbleChartContent() {
  ...
  var selectedRange = rangeSelect.getRange();
...
function addZoom(dims) {
  ...
  gy = svg.select('.y.axis'); // z holds a copy of the previous transform, so we can track its changes

  var z = d3.zoomIdentity;

  xAxis = function xAxis(g, scale) {
    return g.attr("transform", "translate(".concat(dims.left, ",").concat(dims.height + 30, ")")).call(d3.axisBottom(scale).tickSizeOuter(5).tickFormat(function (x) {
      if (x >= 1) {
        return "".concat(x.toFixed(0));
      } else {
        return "";
      }
    }));
  ...
  yAxis = function yAxis(g, scale) {
    return g.attr("transform", "translate(".concat(ORIGINAL_XSCALE.range()[0] + dims.left, ",5)")).call(d3.axisLeft(scale).tickFormat(function (x) {
  ...
  var zoomX = d3.zoom().scaleExtent([0.8, MAX_ZOOM]).translateExtent([[ORIGINAL_XSCALE(1), 0], [ORIGINAL_XSCALE(ORIGINAL_XSCALE.domain()[1] + 1000000), 0]]);
  zoomY = d3.zoom().scaleExtent([0.8, MAX_ZOOM]).translateExtent([[0, ORIGINAL_YSCALE(1) - 100], [0, ORIGINAL_YSCALE(0) + 200]]);
  ...
  zoom = d3.zoom().on("zoom", function () {
    ...
    var doX = point[0] > dims.left;
    var doY = point[1] < dims.height;
...
function redraw() {
  var translation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  currentXScale = tx().rescaleX(ORIGINAL_XSCALE);
  currentYScale = ty().rescaleY(ORIGINAL_YSCALE);
  var animationTime = translation ? 0 : ANIMATION_DURATION;
  gx.transition().duration(animationTime).call(xAxis, currentXScale);
  gy.transition().duration(animationTime).call(yAxis, currentYScale);
  d3.select("#bubbleArea").selectAll(".productCircle").transition().duration(animationTime).attr("cx", function (d) {
    return currentXScale(d.prix);
  }).attr("cy", function (d) {
    return currentYScale(d.disponibility);
  });
...
function addInfoTooltip(svg) {
  var infoChartTip = (0, _d3Tip.default)().attr('class', 'd3-tip').html(function (d) {
  ...
  svg.select('.x.axis-text').on('mouseover', function () {
  ...
  svg.select('.y.axis-text').on('mouseover', function () {
...
function adjustZoomAndOpacity(data) {
  var Y_EXTENT = d3.extent(data, function (d) {
    return d.disponibility;
  });
  var k = (ORIGINAL_YSCALE(0) - ORIGINAL_YSCALE(1)) / (ORIGINAL_YSCALE(Y_EXTENT[0]) - ORIGINAL_YSCALE(Y_EXTENT[1]));
  k = Math.max(0.8, 0.8 * Math.min(k, MAX_ZOOM)); //clamp zoom
  // translate middle of the points to the middle of the viz

  var translateY = dims.height / (2 * k) - (ORIGINAL_YSCALE(Y_EXTENT[1]) + ORIGINAL_YSCALE(Y_EXTENT[0])) / 2;
  var t = d3.zoomIdentity.scale(k).translate(0, translateY);
  gy.transition().duration(ANIMATION_DURATION).call(zoomY.transform, t);
  currentYScale = t.rescaleY(ORIGINAL_YSCALE);
  gy.transition().duration(ANIMATION_DURATION).call(yAxis, currentYScale);
  d3.select("#bubbleArea").selectAll(".productCircle").transition().duration(ANIMATION_DURATION).attr("cx", function (d) {
    return currentXScale(d.prix);
  }).attr("cy", function (d) {
    return currentYScale(d.disponibility);
  }).attr("opacity", function (d) {
    return d.subTotal > 0 ? 1 : 0.1;
  }); //opacity update (problem if done separately)
}

function addResetZoom(svg, updateFct) {
  var offset = 60;
  svg.append('rect').attr('x', dims.full_width - offset).attr('y', 30).attr('rx', 3).attr('ry', 3).attr('width', 30).attr('height', 30).attr('fill', 'rgb(305,255,255)').attr('filter', 'url(#shadow-2)').attr('id', 'bubble-reset-bg');
  svg.append('svg').attr('x', dims.full_width - offset + 3).attr('y', 33).attr('width', 30).attr('height', 30) // .attr('id', 'bubble-reset-path')
  .append('path').attr('id', 'bubble-reset-icon').attr('d', 'M14 12c0-1.1-.9-2-2-2s-2 .9-2 2 .9 2 2 2 2-.9 2-2zm-2-9c-4.97 0-9 4.03-9 9H0l4 4 4-4H5c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.51 0-2.91-.49-4.06-1.3l-1.42 1.44C8.04 20.3 9.94 21 12 21c4.97 0 9-4.03 9-9s-4.03-9-9-9z');
  svg.append('rect').attr('x', dims.full_width - offset).attr('y', 25).attr('rx', 3).attr('ry', 3).attr('width', 30).attr('height', 30).attr('fill', 'transparent') // .attr('id', 'map-reset-btn')
  .on('mouseover', function () {
    d3.select(this).style('cursor', 'pointer');
    d3.select('#bubble-reset-icon').attr('fill', 'grey');
  }).on('mouseout', function () {
    d3.select('#bubble-reset-icon').attr('fill', 'black');
  }).on('click', function () {
    preprocess.resetCategories();
    d3.selectAll(".legendOrdinal .legendCells .cell").style("opacity", 1);
    updateFct();
  }).append('title').text('Réinitialiser les catégories');
}
},{"./tooltips.js":"scripts/bubblechart/tooltips.js","./legend.js":"scripts/bubblechart/legend.js","./helper.js":"scripts/bubblechart/helper.js","./preprocess.js":"scripts/bubblechart/preprocess.js","./range-select.js":"scripts/bubblechart/range-select.js","./viz.js":"scripts/bubblechart/viz.js","d3-tip":"../node_modules/d3-tip/index.js","d3-array":"../node_modules/d3-array/src/index.js"}],"scripts/sankey/preprocess.js":[function(require,module,exports) {
"use strict";
 ...
 */
function display(d, sankeyParametersArray, firstCategories, y) {
  var panel = d3.select('#sankey-panel').style('visibility', 'visible').style('margin-top', Math.min(y + 60, 1230) + 'px');
  ...
  var succ1Info = findInfo(d, sankeyParametersArray[0], firstCategories);
  var succ2Info = findInfo(d, sankeyParametersArray[1], firstCategories);
  var succ1 = panel.append('div').style('font-size', '16px').text(succ1Info.succName + ": " + succ1Info.succQuantity);
  var succ2 = panel.append('div').style('font-size', '16px').text(succ2Info.succName + ": " + succ2Info.succQuantity);
}
/**
...
function findInfo(d, sankeyParameters, firstCategories) {
  ...
  if (sankeyParameters.regionFilter == "all" && sankeyParameters.branchFilter == "all") {
    ...
    if (firstCategories.has(d.name)) {
      succData = sankeyParameters.data.filter(function (e) {
        return e.region == "all" && e.succ_name == "all" && e.source == d.name;
      });
      succData.forEach(function (f) {
        succQuantity += parseInt(f.value);
      });
    } else {
      succData = sankeyParameters.data.find(function (e) {
    ...
    if (firstCategories.has(d.name)) {
      succData = sankeyParameters.data.filter(function (e) {
        return e.region == succName && e.succ_name == "all" && e.source == d.name;
      });
      succData.forEach(function (f) {
        return succQuantity += parseInt(f.value);
      });
    } else {
      succData = sankeyParameters.data.find(function (e) {
        return e.region == succName && e.succ_name == "all" && e.target == d.name;
      });
      succQuantity = succData ? succData.value : 0;
    }
  } else {
    ...
    if (firstCategories.has(d.name)) {
      succData = sankeyParameters.data.filter(function (e) {
        return e.succ_name == succName && e.source == d.name;
      });
      succData.forEach(function (f) {
        return succQuantity += parseInt(f.value);
      });
      ...
      succData = sankeyParameters.data.find(function (e) {
        return e.succ_name == succName && e.target == d.name;
      });
      succQuantity = succData ? succData.value : 0;
    }
  }
...
exports.addSankeyLinksGroup = addSankeyLinksGroup;
exports.addSankeyNodesGroup = addSankeyNodesGroup;
exports.drawSankeyLinks = drawSankeyLinks;
exports.applyLinksConfiguration = applyLinksConfiguration;
exports.drawSankeyNodes = drawSankeyNodes;
exports.applyNodesConfiguration = applyNodesConfiguration;
exports.drawSankeyNodesRects = drawSankeyNodesRects;
exports.applyNodesRectsConfiguration = applyNodesRectsConfiguration;
exports.addSankeyNodeLabels = addSankeyNodeLabels;
exports.applyTextRectConfiguration = applyTextRectConfiguration;
exports.applyTextConfiguration = applyTextConfiguration;
...
function createSankey(data, width, height) {
  return d3.sankey().nodeWidth(20).nodePadding(10).nodeAlign(d3.sankeyLeft).linkSort(function (d1, d2) {
...
function addSankeyLinksGroup(g) {
...
function addSankeyNodesGroup(g) {
  return g.append("g").attr("class", "nodes").attr("font-family", "sans-serif").attr("font-size", 11).selectAll("g");
}

function drawSankeyLinks(link, data) {
  var updatedLinks = link.data(data.links).enter().append("path");
  applyLinksConfiguration(updatedLinks);
}

function applyLinksConfiguration(link) {
  link.attr("d", d3.sankeyLinkHorizontal()).attr("stroke-width", function (d) {
...
function drawSankeyNodes(node, data, highlight, sankeyParametersArray, firstCategories, secondCategoryWithChild) {
  return applyNodesConfiguration(node.data(data.nodes).enter().append("g"), highlight, sankeyParametersArray, firstCategories, secondCategoryWithChild);
}

function applyNodesConfiguration(node, highlight, sankeyParametersArray, firstCategories, secondCategoryWithChild) {
...
function drawSankeyNodesRects(node, color, func) {
  applyNodesRectsConfiguration(node.append("rect").on('click', function (d) {
    func(d.name);
  }), color, func);
}

function applyNodesRectsConfiguration(node, color) {
  node.attr("x", function (d) {
    return d.x0;
  }).attr("y", function (d) {
    return d.y0;
  }).attr("height", function (d) {
    return d.y1 - d.y0 > 4 ? d.y1 - d.y0 : 4;
  }).attr("width", function (d) {
    return d.x1 - d.x0;
  }).attr("fill", function (d) {
    return color(d.firstCat);
  });
}
/**
...
function applyTextRectConfiguration(node) {
  node.attr("class", "textRect").attr("x", function (d) {
    return d.x1 + 3;
  }).attr("y", function (d) {
...
function applyTextConfiguration(node) {
  node.attr("x", function (d) {
    return d.x0 - 6;
  }).attr("y", function (d) {
    return (d.y1 + d.y0) / 2;
...
},{"./panel.js":"scripts/sankey/panel.js"}],"scripts/sankey/sankey-diagram.js":[function(require,module,exports) {
"use strict";
...
function updateGraph(sankeyParameters) {
  ...
  var link = sankeyParameters.g.select(".links").selectAll("path").data(sankeyData.links, function (d) {
    return d;
  });
  ...
  var nodeEnter = viz.applyNodesConfiguration(node.enter().append("g"), highlight, sankeyParametersArray, firstCategories, secondCategoryWithChild);
...
function createGraph(data, g) {
  ...
  var link = viz.addSankeyLinksGroup(g);
  var node = viz.addSankeyNodesGroup(g);
  sankey(data);
...
},{"./preprocess.js":"scripts/sankey/preprocess.js","./helper.js":"scripts/sankey/helper.js","./viz.js":"scripts/sankey/viz.js"}],"scripts/map/viz.js":[function(require,module,exports) {
"use strict";
...
exports.updateRegionRanking = updateRegionRanking;
...
function showSuccCount(svg) {
  ...
  d3.select('#count').html(count === 0 || count === 1 ? '<b>' + count + ' succursale affichée</b>' : '<b>' + count + ' succursales affichées</b>');
}
/**
...
function unselectSucc(currentZoom) {
  ...
  d3.selectAll('#map-viz .cancel').classed('cancelled', false);
...
function drawMap(child, path, data, colorScale, currentZoom, succTypes) {
  child.append('g').selectAll('#map-viz path').data(data.features).enter().append('g').append('path').attr('d', path).style('fill', function (d, i) {
    ...
    showRegionInfoPanel(succTypes, d); // Increase stroke size

    d3.select(this).style('stroke-width', 3 / currentZoom.val).classed('hovered', true); // Highlight matching list entry
...
function showRegionInfoPanel(succTypes, d) {
  d3.select('#map-viz svg').append('rect').attr('x', document.getElementById('svg-map').clientWidth - 314).attr('y', document.getElementById('svg-map').clientHeight - 145).attr('rx', 12).attr('ry', 12).attr('width', 300).attr('height', 110).attr('fill', 'rgb(255,255,255)').attr('filter', 'url(#shadow-1)').attr('class', 'region-info-bg');
  d3.select('#map-viz svg').append('text').attr('x', document.getElementById('svg-map').clientWidth - 304).attr('y', document.getElementById('svg-map').clientHeight - 115).attr('class', 'region-info').style('text-anchor', 'left').style('font-size', 20).text(d.properties.res_nm_reg);
  d3.select('#map-viz svg').append('text').attr('x', document.getElementById('svg-map').clientWidth - 304).attr('y', document.getElementById('svg-map').clientHeight - 85).attr('class', 'region-info').style('text-anchor', 'left').style('font-size', 15).text('- ' + succTypes[d.properties.res_nm_reg].shown + ' succursales affichées'); // select secondary info depending on the mode (by population or area)

  if (d3.select('#switch-map').property('value') === 'Switch 1') {
    d3.select('#map-viz svg').append('text').attr('x', document.getElementById('svg-map').clientWidth - 304).attr('y', document.getElementById('svg-map').clientHeight - 55).attr('class', 'region-info').style('text-anchor', 'left').style('font-size', 15).text('- ' + getPopRatio(succTypes[d.properties.res_nm_reg]) + ' succursales/100 000 hab.');
  } else {
    d3.select('#map-viz svg').append('text').attr('x', document.getElementById('svg-map').clientWidth - 304).attr('y', document.getElementById('svg-map').clientHeight - 55).attr('class', 'region-info').style('text-anchor', 'left').style('font-size', 15).text('- ' + getAreaRatio(succTypes[d.properties.res_nm_reg]) + ' succursales/10 000 km2');
  }
}
/**
...
function filtering(currentZoom, colorScale, succTypes) {
  d3.selectAll('#map-viz .ui-checkbox').on('click', function (d) {
    ...
    if (label.classed('ui-checkbox-on') === true) {
      ...
      if (checkbox.property('value') === 'tout') {
        ...
        d3.selectAll('#map-viz .ui-checkbox label').classed('ui-checkbox-on', false); // d3.selectAll('#map-viz .ui-checkbox input').attr('data-cacheval', function (d, i) { if (i !== 0) return true })
    ...
    if (label.classed('ui-checkbox-off') === true) {
      if (checkbox.property('value') === 'tout') {
        ...
        d3.selectAll('#map-viz .ui-checkbox label').classed('ui-checkbox-on', true); // d3.selectAll('#map-viz .ui-checkbox input').attr('data-cacheval', function (d, i) { if (i !== 0) return false })
...
function addMapMarkers(child, data, panel, currentZoom) {
  child.append('g').selectAll('#map-viz circle').data(data).enter().append('circle').attr('cx', function (d) {
    ...
    d3.selectAll('#map-viz .cancel').classed('cancelled', true);
  }).on('mouseover', function (d) {
...
function updateRegionRanking(succTypes, mode, colorScale) {
  d3.selectAll('#map-viz .para li').remove();
  d3.selectAll('#map-viz .para .trend').remove(); // reset number of shown succursales

  Object.values(succTypes).forEach(function (element) {
    element.shown = 0;
  ...
  sortArray.forEach(function (d, i) {
    d3.select('#map-viz .rank').append('li').html('<span>' + d[0] + '</span><span style="float: right">' + (Math.round(d[1] * 10) / 10).toLocaleString(undefined, {
      minimumFractionDigits: 1
    }) + '</span>').on('mouseover', function (d) {
      d3.select(this).style('font-weight', 'bold');
      var name = d3.select(this).select('span').property('innerHTML'); // send mousover event to corresponding map element
      ...
      d3.select(this).style('font-weight', 'normal');
      var name = d3.select(this).select('span').property('innerHTML'); // send mouseout event to corresponding map element
  ...
  d3.selectAll('#map-viz path.cancel').style('fill', function (d, i) {
    if (i !== 0) {
      if (mode === 0) {
        ...
        return colorScale(Math.log(getAreaRatio(succTypes[d.properties.res_nm_reg])));
      }
    }
  });
}
...
function getPopRatio(regionSuccs) {
  return (Math.round(regionSuccs.shown / regionSuccs.population * 100000 * 10) / 10).toLocaleString(undefined, {
    minimumFractionDigits: 1
  });
}
/**
...
function getAreaRatio(regionSuccs) {
  return (Math.round(regionSuccs.shown / regionSuccs.superficie * 10000 * 10) / 10).toLocaleString(undefined, {
    minimumFractionDigits: 1
  });
}
},{}],"scripts/map/legend.js":[function(require,module,exports) {
"use strict";
...
exports.draw = draw;
exports.setLegendTitle = setLegendTitle;
...
function draw(x, y, height, width, fill, colorScale, mode) {
  ...
  d3.select('#map-viz .legend.axis').attr('transform', 'translate(' + 50 + ',' + 35 + ')').style('font-size', '14px').call(d3.axisLeft(legendScale).ticks(6).tickFormat(function (d) {
...
function setLegendTitle(text) {
  ...
  d3.select('#map-viz svg').append('rect').attr('x', 45).attr('y', 765).attr('rx', 12).attr('ry', 12).attr('width', 255).attr('height', 45).attr('fill', 'rgb(255,255,255)').attr('filter', 'url(#shadow-1)').attr('class', 'legend-title-bg');
  d3.select('#map-viz svg').append('text').attr('x', 50).attr('y', 793).attr('class', 'legend-title').style('text-anchor', 'left').text(text);
}
},{}],"scripts/map/panel.js":[function(require,module,exports) {
"use strict";
...
function addMap() {
  ...
  colorScale = d3.scaleSequential(d3Chromatic.interpolatePuRd); // initialize the svg zoom

  var zoom = d3.zoom().scaleExtent([1, 200]).extent([[0, 0], [svgSize.width, svgSize.height]]).on('zoom', function () {
  ...
  var svg = d3.select('#map-viz').select('svg').attr('width', svgSize.width).attr('height', svgSize.height).append('g').call(zoom); // element hierarchy to fix finicky zoom
  ...
  mapResizeFct = function mapResizeFct() {
    helper.initMapNav(svg, zoom);
  };
}
/**
...
function build(svg) {
  ...
  var DATA = {};
  var promises = [d3.json('./regions.geojson').then(function (data) {
    DATA.geojson = data;
  ...
  Promise.all(promises).then(function () {
    // add svg map
    viz.drawMap(child, path, DATA.geojson, colorScale, currentZoom, succTypes); // convert succursales coordinates
    ...
    viz.addMapMarkers(child, DATA.csv, panel, currentZoom); // show the shown succursales count
...
var mapHelper = _interopRequireWildcard(require("./scripts/map/helper.js"));
...
},{"./scripts/bubblechart/preprocess.js":"scripts/bubblechart/preprocess.js","./scripts/menu.js":"scripts/menu.js","./scripts/bubblechart/helper.js":"scripts/bubblechart/helper.js","./scripts/map/helper.js":"scripts/map/helper.js","./scripts/bubblechart/bubble-chart.js":"scripts/bubblechart/bubble-chart.js","./scripts/sankey/sankey-diagram.js":"scripts/sankey/sankey-diagram.js","./scripts/map/map-viz.js":"scripts/map/map-viz.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
